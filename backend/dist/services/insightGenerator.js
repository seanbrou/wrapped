...
function extractTopStats(service, stats) {
    ...
    for (const group of topItems) {
        ...
        if (items.length > 0) {
            const top3 = items.slice(0, 3).map((i) => `${i.name} (${formatValue(i.count)})`).join(', ');
...
/* ─── Prompt builders ───────────────────────────────────────────────── */
function buildServicePrompt(context) {
...
function buildCrossServicePrompt(allServices) {
...
/* ─── LLM callers ───────────────────────────────────────────────────── */
async function callGeminiDirect(prompt, maxTokens = 2048) {
...
async function tryGateway(prompt) {
    try {
...
function smartDefaultForService(service, stats) {
    const agg = stats.aggregates ?? stats;
    const totals = agg.totals ?? {};
    const streaks = agg.streaks ?? {};
    const topItems = agg.top_items ?? [];
    const getTop = (category) => topItems.find((g) => g.category === category)?.items?.[0]?.name ?? 'Unknown';
    const getTopCount = (category) => topItems.find((g) => g.category === category)?.items?.[0]?.count ?? 0;
    const fmt = (n) => {
        if (n >= 1_000_000)
...
export async function generateAIInsights(context) {
    ...
    try {
        ...
        if (gatewayText) {
            const parsed = parseJsonSafely(gatewayText);
            if (parsed && parsed.heroComparison && parsed.insightHeadline) {
                const defaults = smartDefaultForService(context.service, context.stats);
                return {
                    heroComparison: parsed.heroComparison || defaults.heroComparison,
...
export async function generateCrossServiceInsights(allServices) {
