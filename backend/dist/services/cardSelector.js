...
function percentileFromMetric(value) {
...
function buildChart(service, stats) {
    const chart = stats.aggregates.charts?.[0];
    if (!chart || !chart.data.length || !chart.labels.length)
        return null;
    return {
        id: cardId(),
        type: 'chart',
...
function buildComparison(service, stats, title) {
...
function buildCommunity(service, metric, value, score) {
    return {
        id: cardId(),
        type: 'community',
        service,
...
function buildFunFact(service, fact) {
    return {
        id: cardId(),
        type: 'fun_fact',
        service,
...
function buildStreak(service, streak, description, days) {
    return {
        id: cardId(),
        type: 'streak',
        service,
...
function buildTrend(service, trend, direction, context) {
    return {
        id: cardId(),
        type: 'trend',
        service,
...
function buildSuperlative(service, superlative, stat, value) {
    return {
        id: cardId(),
        type: 'superlative',
        service,
...
function buildOpener(serviceCount, period) {
    return {
        id: cardId(),
        type: 'opener',
        service: 'all',
...
function buildCrossServiceCard(headline, description, services) {
    return {
        id: cardId(),
        type: 'cross_service',
        service: 'all',
...
function buildSpotifyCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('spotify', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('spotify', copy.streakHighlight, 'Consistency matters'));
    if (copy?.superlative)
        cards.push(buildSuperlative('spotify', copy.superlative, 'Top Artist', topArtist));
    const chart = buildChart('spotify', stats);
    if (chart)
        cards.push(chart);
    cards.push(buildCommunity('spotify', 'artist affinity', `${sample} recent plays sampled`, sample));
    if (copy?.trendObservation)
        cards.push(buildTrend('spotify', copy.trendObservation, 'up', 'Listening pattern'));
    if (totalHours > 0) {
        cards.push(buildSuperlative('spotify', 'Total listening time', 'Hours', `${shortNumber(totalHours)}H`));
    }
    return cards;
}
function buildStravaCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('strava', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('strava', copy.streakHighlight, 'Keep the chain going'));
    if (copy?.superlative)
        cards.push(buildSuperlative('strava', copy.superlative, 'Distance', `${shortNumber(Math.round(distanceKm))} km`));
    ...
    if (copy?.trendObservation)
        cards.push(buildTrend('strava', copy.trendObservation, 'up', 'Activity pattern'));
    return cards;
}
function buildFitbitCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('fitbit', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('fitbit', copy.streakHighlight, 'Daily consistency'));
    if (copy?.superlative)
        cards.push(buildSuperlative('fitbit', copy.superlative, 'Total Steps', shortNumber(totalSteps)));
    ...
    if (copy?.trendObservation)
        cards.push(buildTrend('fitbit', copy.trendObservation, 'up', 'Activity trend'));
    return cards;
}
function buildLastfmCards(stats, copy) {
    const artists = stats.aggregates.top_items.find((group) => group.category === 'artists')?.items ?? [];
    const tracks = stats.aggregates.top_items.find((group) => group.category === 'tracks')?.items ?? [];
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('lastfm', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('lastfm', copy.streakHighlight, 'Listening streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('lastfm', copy.superlative, 'Top Artist', topArtist));
    const chart = buildChart('lastfm', stats);
    if (chart)
        cards.push(chart);
    cards.push(buildCommunity('lastfm', 'scrobble depth', `${sample} recent tracks sampled`, sample));
    if (copy?.trendObservation)
        cards.push(buildTrend('lastfm', copy.trendObservation, 'up', 'Listening trend'));
    return cards;
}
function buildSteamCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('steam', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('steam', copy.streakHighlight, 'Gaming streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('steam', copy.superlative, 'Top Game', topGame));
    const chart = buildChart('steam', stats);
    if (chart)
        cards.push(chart);
    cards.push(buildCommunity('steam', 'playtime', `${shortNumber(totalHours)} hours`, totalHours));
    if (copy?.trendObservation)
        cards.push(buildTrend('steam', copy.trendObservation, 'up', 'Gaming trend'));
    return cards;
}
function buildGitHubCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('github', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('github', copy.streakHighlight, 'Code streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('github', copy.superlative, 'Top Repo', topRepo));
    ...
    if (copy?.trendObservation)
        cards.push(buildTrend('github', copy.trendObservation, 'up', 'Contribution trend'));
    return cards;
}
function buildNotionCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('notion', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('notion', copy.streakHighlight, 'Editing streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('notion', copy.superlative, 'Workspace', workspaceName));
    ...
    if (copy?.trendObservation)
        cards.push(buildTrend('notion', copy.trendObservation, 'up', 'Organization trend'));
    return cards;
}
function buildAppleHealthCards(stats, copy) {
    const totalSteps = stats.aggregates.totals.totalSteps ?? 0;
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('apple_health', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('apple_health', copy.streakHighlight, 'Health streak', longestExerciseStreak || undefined));
    if (copy?.superlative)
        cards.push(buildSuperlative('apple_health', copy.superlative, 'Top Workout', topWorkoutType));
    ...
    if (copy?.trendObservation)
        cards.push(buildTrend('apple_health', copy.trendObservation, 'up', 'Health trend'));
    return cards;
}
function buildTraktCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('trakt', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('trakt', copy.streakHighlight, 'Binge streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('trakt', copy.superlative, 'Top Show', topShow));
    const chart = buildChart('trakt', stats);
    if (chart)
        cards.push(chart);
    cards.push(buildCommunity('trakt', 'watch time', `${shortNumber(totalHours)} hours`, totalHours));
    if (copy?.trendObservation)
        cards.push(buildTrend('trakt', copy.trendObservation, 'up', 'Viewing trend'));
    return cards;
}
function buildRedditCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('reddit', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('reddit', copy.streakHighlight, 'Engagement streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('reddit', copy.superlative, 'Karma', shortNumber(totalKarma)));
    const chart = buildChart('reddit', stats);
    if (chart)
        cards.push(chart);
    cards.push(buildCommunity('reddit', 'karma power', `${shortNumber(totalKarma)} karma`, totalKarma));
    if (copy?.trendObservation)
        cards.push(buildTrend('reddit', copy.trendObservation, 'up', 'Engagement trend'));
    return cards;
}
function buildRescueTimeCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('rescuetime', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('rescuetime', copy.streakHighlight, 'Focus streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('rescuetime', copy.superlative, 'Pulse', String(avgPulse)));
    const chart = buildChart('rescuetime', stats);
    if (chart)
        cards.push(chart);
    cards.push(buildCommunity('rescuetime', 'productivity', `${avgPulse} pulse`, avgPulse));
    if (copy?.trendObservation)
        cards.push(buildTrend('rescuetime', copy.trendObservation, productiveHours > distractingHours ? 'up' : 'down', 'Productivity trend'));
    return cards;
}
function buildTodoistCards(stats, copy) {
    ...
    if (copy?.funFact)
        cards.push(buildFunFact('todoist', copy.funFact));
    if (copy?.streakHighlight)
        cards.push(buildStreak('todoist', copy.streakHighlight, 'Completion streak'));
    if (copy?.superlative)
        cards.push(buildSuperlative('todoist', copy.superlative, 'Tasks', shortNumber(tasksCompleted)));
    ...
    if (copy?.trendObservation)
        cards.push(buildTrend('todoist', copy.trendObservation, 'up', 'Completion trend'));
    return cards;
}
const builders = {
...
export function selectCards(stats, copyByService = {}, crossServiceInsights = [], maxCards = 25) {
    const cards = [];
    // 1) Opener card
