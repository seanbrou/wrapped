...
function policyFor(service: ServiceId) {
  switch (service) {
    case 'spotify':
    case 'strava':
    case 'fitbit':
...
function nextIsoFromMs(msFromNow: number) {
  return new Date(Date.now() + msFromNow).toISOString();
}

function retryDelayMs(attempt: number, retryAfterMs?: number) {
...
async function processJob(job: Awaited<ReturnType<typeof syncJobs.claimNextRunnable>>) {
  if (!job) return;
